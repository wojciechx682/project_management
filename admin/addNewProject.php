<?php
    ...
    if (empty($_POST["title"]) || empty($_POST["description"]) || empty($_POST["status"]) ||
        empty($_POST["start_date"]) || empty($_POST["end_date"]) || empty($_POST["team_id"])) {
    ...
    $insertSuccessful = query(
        ...
        "addNewProject",
        [$title, $description, $startDate, $endDate, $statusFormatted, $teamId]
    );
    ...
    $teamName = query("SELECT team.name FROM team WHERE team.id=?", "getTeamName", $teamId);
